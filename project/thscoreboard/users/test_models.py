...
class InviteTestCase(test.TestCase):
    ...
    def testInviteUser(self):
        invite = models.InvitedUser.CreateInvite(
            "some-username", "some-email@example.com", self._staff
        )
        ...
        self.assertTrue(real_user.check_password("some-password"))
...
class BanTestCase(test_case.UserTestCase):
    ...
    def testBanUser(self):
        b = self.target.BanUser(
            self.banner, "test", datetime.timedelta(hours=6), expiration=self.now
        )
        ...
        self.assertAlmostEqual(
            b.expiration, self.now, delta=datetime.timedelta(minutes=1)
        )
    ...
    def testBanUserComputesExpirationTime(self):
        b = self.target.BanUser(self.banner, "test", datetime.timedelta(hours=6))
        self.assertAlmostEqual(
            b.expiration,
    ...
    def testCleanUpBannedUser_IsXBannedRespectExpirationTime(self):
        ...
        self.assertFalse(
            models.Ban.IsUsernameBanned(
                "target", now=self.now + datetime.timedelta(days=3)
            )
        )
        self.assertFalse(
            models.Ban.IsEmailBanned(
                target_email, now=self.now + datetime.timedelta(days=3)
            )
        )
...
class DeletedUserTest(test_case.UserTestCase):
    ...
    def testMarkForDeletion(self):
        ...
        self.assertAlmostEqual(
            self.user.deleted_on, self.now, delta=datetime.timedelta(minutes=1)
        )
    ...
    def testCleanUpDeletesMarkedUsers(self):
        ...
        recently_deleted_user = models.User.objects.get(
            username="recently-deleted-user"
        )
