    ...
    if request.method == 'POST':
        ...
        if not button_value:
            ...
            if 'invite_file' in request.FILES:
                ...
                confirm_form = forms.UploadInviteFileConfirmationForm(initial={
            ...
            confirm_form = forms.UploadInviteFileConfirmationForm(request.POST, request.FILES)
            if confirm_form.is_valid():
                parsed_csv = parse_invite_csv.Parse(confirm_form.cleaned_data['invite_file_contents'])

                for r in parsed_csv:
                    if r.errors:
                        logging.info('Parsed CSV had errors; not uploading.', parsed_csv.errors_str)
