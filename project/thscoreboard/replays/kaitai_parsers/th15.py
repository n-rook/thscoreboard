...
class Th15(KaitaiStruct):
    def __init__(self, _io, _parent=None, _root=None):
        super(Th15, self).__init__(_io)
        self._parent = _parent
        self._root = _root or self
        self._read()

    def _read(self):
        self.header = Th15.Header(self._io, self, self._root)
        self.stages = []
        for i in range(self.header.stage_count):
            self.stages.append(Th15.Stage(self._io, self, self._root))
    ...
    class Header(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Th15.Header, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class Stage(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Th15.Stage, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
        ...
        def _read(self):
            ...
            self.time_in_chapter = self._io.read_u4le()
            self.shot = self._io.read_u4le()
            self.subshot_unused = self._io.read_u4le()
            self.score = self._io.read_u4le()
            self.difficulty = self._io.read_u4le()
            ...
            self.graze_chapter = self._io.read_u4le()
            self.spell_practice_id = self._io.read_u4le()
            self.unknown_2 = self._io.read_u4le()
            ...
            self.enemies_in_chapter = self._io.read_u4le()
            self.enemies_killed_in_chapter = self._io.read_u4le()
            self.bgm_fn = self._io.read_bytes(256)
            self.unknown_8 = self._io.read_u4le()
            self.unknown_9 = self._io.read_u4le()
            self.pd_resets_total = self._io.read_u4le()
            self.pd_resets_stage = []
            for i in range(8):
                self.pd_resets_stage.append(self._io.read_u4le())

            self.pd_resets_chapter = self._io.read_u4le()
        ...
        def _fetch_instances(self):
            ...
            for i in range(len(self.pd_resets_stage)):
                pass
