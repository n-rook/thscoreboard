...
class Th12(KaitaiStruct):
    ...
    def _read(self):
        ...
        for i in range(self.header.stagecount):
            self.stages.append(Th12.Stage(self._io, self, self._root))
    ...
    class Header(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Th12.Header, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
    ...
    class Stage(KaitaiStruct):
        def __init__(self, _io, _parent=None, _root=None):
            super(Th12.Stage, self).__init__(_io)
            self._parent = _parent
            self._root = _root
            self._read()
