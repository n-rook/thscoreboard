        ...
        if form.is_valid():
            try:
                ...
                temp_replay = models.TemporaryReplayFile(
                    user=request.user, replay=file_contents
                )
                temp_replay.save()

                return redirect(publish_replay, temp_replay.id)
...
def publish_replay(request, temp_replay_id):
    ...
    try:
        temp_replay = models.TemporaryReplayFile.objects.get(
            user=request.user, id=temp_replay_id
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            new_replay = create_replay.PublishReplayWithoutFile(
                ...
                route=form.cleaned_data.get(
                    "route"
                ),  # Passes None if route is not defined.
                score=form.cleaned_data["score"],
                category=form.cleaned_data["category"],
                is_clear=form.cleaned_data["is_clear"],
                comment=form.cleaned_data["comment"],
            ...
            return render(
                ...
                {"game": game, "form": form, "has_replay_file": False},
            )
