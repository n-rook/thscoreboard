...
def _get_url():
    ...
    return "https://discord.com/api/webhooks/{id}/{token}".format(
        id=settings.DISCORD_WEBHOOK_ID, token=settings.DISCORD_WEBHOOK_TOKEN
    )
...
def announce(message: str):
    ...
    if not has_discord():
        logging.info(
            "The discord webhook is not configured, so we cannot announce anything."
        )
        return

    r = requests.post(_get_url(), params={"wait": "true"}, json=_get_payload(message))
    r.raise_for_status()
